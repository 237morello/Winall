...
/**
 * PAGE : Support Client
 * Permet au client de discuter avec l'equipe de support Winall Tech.
 */
export default async function ClientChatPage() {
  ...
  const role =
    ((session?.user as { role?: string })?.role?.toLowerCase() as
      | "admin"
      | "client") || "client";
      ...
      <div className="flex flex-col gap-4 border-b pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">
            {"Posez vos questions, partagez un besoin technique ou suivez un projet avec l'equipe Winall."}
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:w-[360px]">
          ...
          <Card className="rounded-lg border-border/70 p-3 shadow-none">
            <div className="flex items-center gap-2 text-primary">
              ...
              <span className="text-xs font-medium">Canal securise</span>
            </div>
          </Card>
        </div>
      </div>
      ...
      {adminId ? (
        ...
        <Card className="flex min-h-96 flex-col items-center justify-center rounded-lg border-dashed p-8 text-center shadow-none">
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
            {"Aucun compte administrateur support n'est disponible pour demarrer une conversation."}
          </p>
        </Card>
      )}
    </div>
