...
export function AuthForm() {
  ...
  const [otpEnvoye, setOtpEnvoye] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    if (!racine) return;
    if (modeOuvert === "magic-link") ServiceFormulaireAuth.focaliser('input[aria-label="Email pour recevoir un magic link"]', racine);
    if (modeOuvert === "otp" && !otpEnvoye) ServiceFormulaireAuth.focaliser('input[aria-label="Email pour recevoir un code OTP"]', racine);
    if (modeOuvert === "otp" && otpEnvoye) ServiceFormulaireAuth.focaliser('input[aria-label="Code OTP a 6 chiffres"]', racine);
  }, [modeOuvert, otpEnvoye]);
  ...
  const mutationEnvoiOtp = useMutation({
    ...
    onSuccess: (_, valeurs) => {
      ...
      toast.success("Un code OTP a ete envoye.");
    },
    onError: (erreur: { message?: string } | null) =>
      toast.error(ServiceAuth.normaliserMessageErreur(erreur, "Impossible d'envoyer le code.")),
  });
  ...
  const mutationVerificationOtp = useMutation({
    ...
    onSuccess: async ({ error }) => {
      if (error) return toast.error(ServiceAuth.normaliserMessageErreur(error, "Code invalide."));
    ...
    onError: (erreur: { message?: string } | null) =>
      toast.error(ServiceAuth.normaliserMessageErreur(erreur, "Verification OTP impossible.")),
  });
  ...
  const mutationMagicLink = useMutation({
    ...
    onSuccess: ({ error }) => {
      ...
      toast.success("Votre lien de connexion a ete envoye.");
    },
    onError: (erreur: { message?: string } | null) =>
  ...
  }, [otpValeur, mutationVerificationOtp]);
      ...
      <Card className="gap-0 overflow-hidden border-black/8 bg-white/95 py-0 shadow-xl shadow-black/6 backdrop-blur-sm">
        ...
        <CardHeader className="space-y-1 border-b border-black/5 bg-white px-4 py-3">
          ...
          <CardDescription className="text-xs leading-snug">
            Choisissez Google, GitHub, OTP ou Magic Link.
          </CardDescription>
        </CardHeader>
          ...
          <Tabs value={modeOuvert} onValueChange={setModeOuvert} className="w-full gap-0">
            ...
            <TabsContent value="otp" className="mt-0">
              <PanneauOtp
                emailActif={emailOtp}
                compteur={compteur}
                estVerificationVisible={otpEnvoye}
